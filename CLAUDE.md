...
### 第二の取得元: horror2ch.com(洒落怖まとめ厳選)

オーナーがよく参照するまとめ。「洒落怖厳選」タグに良作が多い。扱いの注意:

- wp-json は WAF で 403。HTML を取得する(collect.py が Cocoon テーマの
  entry-content を入れ子対応で切り出し、ポチップ広告ブロックも自動除去する)
- 長編は `記事URL/2` `記事URL/3` … に分割される。catalog の `urls:` に全ページを列挙する
- ページはサイドバーに**他作品の抜粋**を含む巨大HTML。全文への素朴な正規表現は誤爆する
- 記事によってはレスヘッダの無い整形版なので、その旨を trivia に書く

### 表記ゆれ・別題の重複に注意(オーナー指示)

まとめサイト間で**同じ話が別タイトルで**、また**別の話が同じようなタイトルで**流通している。
新規収録の前に必ず:

1. 冒頭の文・レス日付・ID を既存の `stories/*.md` と突き合わせる
2. dangi.link API で本文の特徴的なフレーズを検索し、別題で収録済み/収録可能でないか確認する
3. 同一と判明したら片方だけ収録し、もう一方の題は aliases と trivia に記録する

**同一判定は、必ず本文div抽出後のテキスト同士で行うこと。**
過去の失敗: horror2ch「閉鎖された村」のページを全文走査したら、サイドバーに出ていた
別記事「悪魔の最大の目的」の抜粋を本文と取り違え、同一作と誤判定した(オーナー指摘で訂正)。
実際は「閉鎖された村」= 2014年のVIP系実況、「悪魔の最大の目的」= 2007年の洒落怖で、全くの別作品。

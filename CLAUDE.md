...
- **捜索で分かった経緯・豆知識は catalog の `trivia:`(箇条書き)に必ず書く**。
  作品ページ末尾に「覚え書き」として注釈表示される(オーナー指定の標準運用)。
  書くのは例えば: 初出サイトと年、転載・再掲の経緯、亜種と原典の区別、
  収録にあたっての判断(底本の選定理由、除去した混入物、他アーカイブとの照合結果)、
  映画化などの後日情報。裏の取れない通説は「〜とされる」と書く
